...
```
L7  PRESENTATION    waste teal + earth brown (Open Design)
L6  ORCHESTRATION   Hermes sub-context (Kimi K2.6 / DeepSeek V3.5 / local)
L5  DOMAIN MCP      muckaway-ai-mcp
L4  AGENT MEMORY    agentmemory (v0.9.26) — shared mode
L3  KNOWLEDGE GRAPH Cognee subgraph — scope: UK skip/grab-hire, landfills, council permits, fleet capacity
L2  VERSIONED HIST  Memoria (v0.4.0) — namespace "muckaway"
...
```bash
git clone https://github.com/CSOAI-ORG/muckaway-hive
cd muckaway-hive
...
`https://muckaway.ai/.well-known/agent-card.json` — see `agent-card.json`.
...
5-10% marketplace + £99-499/mo per rental co.
